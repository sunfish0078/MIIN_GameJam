...
public class FilmDropAnimation : MonoBehaviour
{
    ...
    [Header("Film Slide Down Settings")] 
    ...
    [Header("Photo Slide-In Settings")]
    ...
    private void Awake()
    {
        ...
        filmTargetPos = film.anchoredPosition;
        film.anchoredPosition = filmTargetPos + new Vector2(0f, dropDistance);
    }
    ...
    private void PlayPhotoSlideIn()
    {
        ...
        Vector2 photoTargetPos = photo.anchoredPosition; //slide후 최종 도착지점
        Vector2 photoStartPos = photoTargetPos + new Vector2(-photoSlideDistance, 0f); //도착지점에서 왼쪽으로 뺀다(시작시점구하는)

        photo.anchoredPosition = photoStartPos; //그후 사진을 시작시점으로 일단 고정시킴
        ...
        photo.DOAnchorPos(photoTargetPos, photoSlideDuration)
            ...
            .OnComplete(PlayIntroAnimation); //끝난후 필름 내려온다
    }
    ...
    private void PlayIntroAnimation()
    {
        ...
        // 위치 이동 
        sequence.Join(film.DOAnchorPos(filmTargetPos, slideDuration).SetEase(slideEase));
